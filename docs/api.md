...
| warehouse_id | int | 是| 非管理员此项必填 |
| skill_picking | int | 是| 拣货能力点，默认 0 |
| skill_staging | int | 是| 备货能力点，默认 0 |
| skill_shipping | int | 是| 发货能力点，默认 0 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| qty_on_hand | int | 是| 修正后的现存量（>= 0）|
| qty_threshold | int | 是| 修正后的库存阈值（>= 0）|
| reason | string | 是 | 修正原因，最长 500 字 |

**说明:**
- `qty_on_hand` 和`qty_threshold` 至少传一个。
...
- 禁用仓库与下架商品不允许盘点修正。
- 修正会写入`stocktakes` `inventory_movements`。
...
| qty | int | 是| 进货数量（> 0）|
| reason | string | 是 | 可选说明，最长 500 字 |

**说明:**
- 若仓库中不存在该商品库存记录，系统会自动创建丢条库存记录后再累加入库数量。
- 入库会写入`stocktakes` `inventory_movements`，用于后续审计与追踪。
- 禁用仓库与下架商品不允许进货。
...
- `delta_reserved` `delta_locked` 仅用于业务内部状态管理，不纳入流水趋势统计。
...
- `related_description` 为业务化文案（订单号、调度 from/to、SKU、数量原因等）。
...
- 客户列表返回字段`is_active`，前端可展示停用记录并进行恢复。
...
**详情接口说明:**
- `GET /api/admin/products/{product_id}`：返回单个产品详情，字段结构同产品列`items[]` 元素。

**图片接口说明:**
- 上传图片：`POST /image`，`multipart/form-data`，字段名`image`。
...
```http
GET /api/admin/orders?page=1&page_size=10&search=关键词status=in_progress&start_date=2026-03-01&end_date=2026-03-19
```
...
| priority | string | 是| `high \| medium \| low`，默`medium` |
| description | string | 是| 备注 |
| items | array | 是| 明细数组，至少1条|
...
**说明:**
- `csv`：`text/csv; charset=utf-8`，内容已包含 UTF-8 BOM，Excel 打开中文不乱码。
- `markdown`：`text/markdown; charset=utf-8`。
- `pdf`：`application/pdf`，响`data.content_base64` 为 PDF 的 base64 内容。
...
**说明:**
- 返回 `application/pdf`，响`data.content_base64` 为订单详情 PDF 的 base64 内容。
...
**说明:**
- 返回管理员览页所霢的聚合指标与图表数据（订单运营口径，不含工单趋势图）。
- `product_popularity` 仅统`status = completed` 的订单明细。
...
**说明:**
- 用于总览页仓库订单完成率”图点击下钻。
- 返回仓库汇（总单完成单量/完成率）和调度员明细。

---

## 调度员模块接口（Dispatcher

> 认证要求：以下接口均霢 `Authorization: Bearer <token>`，且当前用户角色必须`dispatcher`。
...
**说明:**
- 仅返`status = pending_acceptance` 的订单。
...
- 返回朢新订单详情对象（结构同详情接口）。
- 若库存可用量不足，返回`400`，并`data.shortages` 中给出商品缺口明细（基于 `available` 对比）。
...
**说明:**
- 用于调度员布屢顶部状徽标侧边栏“我的订单数字工作台聚合统计展示。
...
**说明:**
- `worker_skill < required_skill_min`，后端返回`400`，该工人禁止派单。
- `required_skill_min <= worker_skill < required_skill_max`，返回技能风险（`skill_gap`），允许强制派单。
- `worker_skill >= required_skill_max`，技能维度无风险。
...
| worker_id | int | 是| 工人 ID |
| priority | string | 是| `high \| medium \| low`，默`medium` |
| deadline | string(datetime) | 是| 截止时间 |
| description | string | 是| 工单备注 |
| override_reason | string | 是| 存在风险时必填；强制派单原因 |

**说明:**
- 遵循阶段串行与关门规则：已完成阶段不能新建工单，下一阶段霢上一阶段完成后才可创建。
- 若预校验存在风险且未`override_reason`，创建接口返回`400`。
- 风险放行时，系统会将结构化审计前缢写入 `description`，格式：
  - `[override][skill_gap,worker_overload] 强制原因文本`
  - 若同时填写普通备注，普备注会换行追加在前缢后。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| remark | string | 是| 手动完成原因，长度1~1000 |
...
```http
GET /api/dispatcher/my-orders?search=关键词status=completed
```
...
**说明:**
- 用于调度员我的订单/ 已完成订单分视图加载。

### 16) 调度员工单中心列表（新增。

```http
GET /api/dispatcher/work-orders?search=关键词status=in_progress&sort_by=updated_at&sort_order=desc
```
...
| page | int | 1 | 当前页码，最小为1 |
| page_size | int | 10 | 每页条数，1~100 |
| search | string | - | 按订单号/工人调度员名/仓库名搜|
| status | string | - | `pending \| in_progress \| completed \| terminated` |
| stage_type | string | - | `picking \| staging \| shipping` |
| priority | string | - | `high \| medium \| low` |
| warehouse_id | int | - | 按仓库过|
| worker_id | int | - | 按工人过|
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| cancellation_reason | string | 是| 取消原因，1~500字 |
...
> 认证要求：以下接口均霢 `Authorization: Bearer <token>`，且当前用户角色必须`worker`。
...
- 当工单由调度员风险放行创建时，`description` 可能包含结构化前缢。
...
```json
{
  ...
  "status": "in_progress",
  "priority": "high",
  "description": "[override][skill_gap] 紧补单\n优先处理 A 区货架,
  "source": "manual",
...
| 字段 | 类型 | 说明 |
|------|------|------|
| worker_stage_skill | int | 当前工单阶段下，工人自身抢能|
...
- `current_stage_required_skill`：当前工单阶段对应的技能要求。
- `worker_stage_skill`：当前工单阶段下的工人技能。
- `is_skill_matched`：`worker_stage_skill >= current_stage_required_skill` 时为 `true`

**说明:**
- 仅允许查看当前登录工人自己被分配的工单详情，否则返回 `404`。
- `description` 若包含风险放行结构化前缀，前端应同时展示“风险类放行原因/备注正文”
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| note | object | 否 | 工单备注对象 |

`note` 子字段：
- `note_type`: `normal \| damaged \| qty_mismatch \| other`
- `content`: string，长度1~2000
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| unread_only | boolean | false | 是否仅返回未读 |
| limit | int | 50 | 返回条数，范围 1~200 |
...
```json
{
  ...
  "data": {
    "items": [
      {
        "id": 101,
        "type": "work_order_exception_severe",
        "title": "工单异常：数量不符",
        "body": "订单 OD-260325-001 的工单 #49 出现异常备注（qty_mismatch）：111",
        "related_id": 49,
...
```http
PATCH /api/auth/me/notifications/{notification_id}/read
```

### 全部标记已读

```http
PATCH /api/auth/me/notifications/read-all
```

### 通知 WebSocket（仅通知功能使用）

```http
WS /api/auth/me/notifications/ws?token=<JWT>
```

**说明：**
- 仅通知状态同步走 WebSocket，其他业务接口仍走 HTTP。
- 当通知状态变化时，服务器推送：
...
- 管理员通知抽屉：支持 `全部 / 仅未读 / 仅已读`。
- 调度员异常中心：支持 `全部 / 仅未读 / 仅已读`。
