这次更新**没有增加或删除任何底层数据库字段**，因为咱们这次修正的是**“应用层业务规则（代码拦截逻辑）”**。

我已经在下方的完整版文档中，对 **`9. order_stages（订单阶段表）`** 里的“手动标记”前置条件进行了精准修正，加入了对 `pending` 状态的拦截要求。

你可以直接复制以下完整内容，替换掉你现有的 `database.md` 文件：
...
## 索引建议
*(省略了常规的身份与状态索引，与原版一致，仅去除了已删除技能表的索引)*
