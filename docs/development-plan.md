...
#### 1.3 前端项目初始化
- [ ] 安装路由（Vue Router）和状态管理（Pinia）
- [ ] 安装 UI 框架（推荐 Element Plus 或 Naive UI）
- [ ] 配置 Axios 请求封装（统一鉴权 Header、错误处理）
- [ ] 实现基础布局：侧边栏 + 顶部导航栏 + 内容区
...
#### 2.3 前端认证页面
- [ ] 登录页（表单验证、错误提示、Token 存储）
- [ ] 路由守卫（未登录跳转登录页、按角色限制路由）
- [ ] 用户管理页面（列表、新增弹窗、编辑弹窗、状态切换）
